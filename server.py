...
def register():
    if request.method == 'POST':
        try:
            ...
            hashed_password = bcrypt.generate_password_hash(password=user_data['password'].encode('utf-8')).decode('utf-8')
            new_user = User(first_name=user_data['first_name'], last_name=user_data['last_name'], email=user_data['email'], phone_number=user_data['phone'], password=hashed_password)
    ...
    if request.method == 'POST':
        try:
            ...
            if not bcrypt.check_password_hash(user.password, user_data['password']):
                return jsonify({"error":"Invalid Credentials"}), 401
            return jsonify({'message':'User logged In', 'id':user.id})
        except:
            return jsonify({'error':'Something went wrong'}), 500
    # return {
...
def usersList(user_id):
    if request.method == 'DELETE':
        ...
        if user_to_delete is not None:
            ...
            db.session.commit()
            return jsonify({'message':f'User {user_to_delete.id} deleted successfully'}),200
        else:
...
def getAllUsers():
    ...
    users = [{'id':user.id, 'first_name':user.first_name, 'last_name':user.last_name, 'email':user.email, 'phone_number':user.phone_number} for user in user_list]
    return jsonify({
